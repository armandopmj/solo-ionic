...
gulp.task('environment', function () {
  return gulp.src('app/config/config.constant.js')
    .pipe(
      $.inject(
        gulp.src('app/config/env-' + options.env + '.js'),
        {
          starttag: '/*inject-env*/',
          endtag: '/*endinject*/',
          transform: function (filePath, file) {
            var json;
            try {
              json = require('../app/config/env-' + options.env + '.js')
              if (Object.values(json).every(function (v) { return v === undefined; })) {
                throw new Error('no env-'+options.env+'.js vars detected, check (.)env.')
              }
            }
            catch (e) {
              console.log(e);
    ...
    .pipe(gulp.dest('app/config-gen/'));
});

gulp.task('build-vars', ['environment'], function () {
  return gulp.src('app/config-gen/config.constant.js')
    .pipe(
      $.inject(
        gulp.src(''),
        {
