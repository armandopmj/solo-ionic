...
var bsInit = function(paths, openOverride) {
    ...
    if (options.env === 'proxy') {
        bsOptions.server.middleware = proxyMiddleware('/api', {
            target: 'http://SOME_PROXY_URL.com',
            changeOrigin: true
        });
    }
...
gulp.task('watch', ['inject-all'], function() {
    ...
    // FIXME watching styles is reloading twice
    gulp.watch('app/**/*/*.scss', ['styles', 'inject-all']);
